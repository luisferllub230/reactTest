 ...
 */
define(['hogan', 'text', 'labels-renderer'], function (hogan, text, labelsRenderer) {
    ...
    function load(name, req, onLoad, config) {
        ...
        text.get(req.toUrl(fileName), function (data) {
            ...
            labelsRenderer.replaceLabels(data).then(function (templateWithLabels) {
                if (config.isBuild) {
                    // store compiled function if build
                    // and should always be a string
                    compilationOptions.asString = true;
                    _buildMap[name] = hogan.compile(templateWithLabels, compilationOptions);
                }

                // maybe it's required by some other plugin during build
                // so return the compiled template even during build
                var template = hogan.compile(templateWithLabels, compilationOptions);
